function encoder = trainEncoder(images, feat, varargin)
...
for i = 1:numImages
    ...
    features = CalculateFeatureCNN(images{i}, feat{i}, net);
    randn('state',0) ;
    rand('state',0) ;
...
switch encoder.type
    case {'crosemodel'}
        [encoder.mean, encoder.covariance, encoder.weight] = tempbuild(descrs);
    case {'bovw', 'vlad'}
    ...
    case {'fv','cnn-fv','cnn-gradient-fv','cnn-vlag-gmm'} ;
        vl_twister('state', opts.seed) ;
        if 1
            v = var(descrs')' ;
